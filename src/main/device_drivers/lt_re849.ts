...
const DEVICE_MODEL = 'LT-RE849';
const LT_RE849_SLAVE_ID = 0x01;

const LT_RE849_DRIVER_CONFIG = [
    'READ_FLOW',
    'READ_PR1',
    'READ_PR2',
    'READ_HUMD1',
...
ipcMain.handle(`${DEVICE_MODEL}_get_device_config`, () => LT_RE849_DRIVER_CONFIG);
ipcMain.handle(`${DEVICE_MODEL}_get_device_cmd_help`, () => LT_RE849_DEVICE_CMD_HELP);
...
const LT_RE849_DEVICE_CMD_HELP: string[] = [
    '================================================================================================================================',
...
const __pool_data_size = (LT_RE849_DRIVER_CONFIG.length - 3) * 4 + 6;
const __pool_filter_set = new Set(['INPUT_REG', 'CTRL_REG', 'FAULT_REG']);
const __pool_msg_config = LT_RE849_DRIVER_CONFIG.filter(x => !__pool_filter_set.has(x.msg_name));
async function __pool_task() {
    while (__enable_pool) {
        ...
        const fltsq_data_buffer = LtBusDriver.concat_uint8_arrays([data_buffer.slice(0, 0x078), data_buffer.slice(0x07A, 0x08A)]);
...
export function init_lt_re849_serial_adapter(_main_window: BrowserWindow) {
    main_window = _main_window;

    ipcMain.on(`${DEVICE_MODEL}_serial_port_connect`, (_, data) => {
        const { port_name } = data;
        lt_bus_driver = new LtBusDriver(port_name, LT_RE849_SLAVE_ID, DEVICE_MODEL, 1, 1000, mw_ipc_handler, mw_logger);
