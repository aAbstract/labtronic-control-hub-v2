...
export function compute_crc16(buffer: Uint8Array): number {
...
export async function ltdsp_request(serial_port: SerialPort, request_packet: Uint8Array, pool_freq_ms: number = 10, timeout: number = 1000): Promise<Result<number[]>> {
    const buffer: number[] = [];
    ...
    function __pool() {
        ...
        const bytes: number[] = serial_port.read(1);
        if (bytes === null)
            return;
        const byte: number = bytes[0];
        buffer.push(byte);
