...
const DEVICE_CPS: CHXComputedParam[] = [
    ...
    { param_name: 'V_air', expr: '44.63 * $VAVG_VMAX * $CP * Math.sqrt(($PR1/1000) / (0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15)))', msg_type: 41, unit: '[m/s]' },
    { param_name: 'Q_air', expr: '60000 * Math.PI * $R * $R * (44.63 * $VAVG_VMAX * $CP * Math.sqrt(($PR1/1000) / (0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15))))', msg_type: 42, unit: '[m3/s]' },
    { param_name: 'D_fuel', expr: '$DO * $SG', msg_type: 43, unit: '[Kg/m3]' },
    { param_name: 'Air_Fuel_Ratio', expr: '(Math.PI * $R * $R * (44.63 * $VAVG_VMAX * $CP * Math.sqrt(($PR1/1000) / (0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15)))) * 0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15) * 1000) / (0.0016 * $RPM * 3.78541 / 3600 * $DO * $SG)', msg_type: 44 },
    ...
    { param_name: 'Fuel_Consumption', expr: '(0.0016 * $RPM * 3.78541 / 3600 * $DO * $SG) / (9.8 * ($L2 - $L1) * $D * 2 * Math.PI * $RPM / 60)', msg_type: 47 },
    ...
    { param_name: 'Load_Diff', expr: ' ($L2 - $L1) ', msg_type: 50, unit: '[kgf]' },
    { param_name: 'Fuel_Rate', expr: '0.0016 * $RPM * 3.78541 / 3600 * $DO * $SG', msg_type: 51, unit: '[g/s]' },
    { param_name: 'Engine_Torque', expr: '($L2 - $L1) * $D', msg_type: 52, unit: '[kgf.m]' }
];
...
function lt_au450_cmd_exec(cmd: string) {
    ...
    if (cmd_parts[0] === 'SET' && input_consts.includes(cmd_parts[1]) && !isNaN(new_set_val)) {
        ...
        save_chx_settings()
        if (lt_au450_vce0)
            lt_au450_vce0.load_symbol('$' + cmd_parts[1], new_set_val);
