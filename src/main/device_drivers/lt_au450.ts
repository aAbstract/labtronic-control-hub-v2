...
const DEVICE_CPS: CHXComputedParam[] = [
    ...
    { param_name: 'Q_air', expr: 'Math.PI * $R * $R * (44.63 * $VAVG_VMAX * $CP * Math.sqrt($PR1 / (0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15))))', msg_type: 42, unit: '[m3/s]' },
    { param_name: 'D_fuel', expr: '$DO * $SG', msg_type: 43, unit: '[Kg/m3]' },
    { param_name: 'Air_Fuel_Ratio', expr: '(Math.PI * $R * $R * (44.63 * $VAVG_VMAX * $CP * Math.sqrt($PR1 / (0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15)))) * 0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15) * 1000) / (0.0016 * $RPM * 3.78541 / 3600 * $DO * $SG)', msg_type: 44 },
    ...
    { param_name: 'Volumetric_Efficiency', expr: '(60000 * (Math.PI * $R * $R * (44.63 * $VAVG_VMAX * $CP * Math.sqrt($PR1 / (0.4638 * $barometric_pressure * 7.50062 / ($T1 + 273.15))))) * $SF) / ($RPM * $SV)', msg_type: 48 },
