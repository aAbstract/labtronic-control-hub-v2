...
const LT_RE850_DRIVER_CONFIG: MsgTypeConfig[] = [
    {
        msg_type: 0,
...
const LT_RE850_VCE_CONFIG = VirtualComputeEngine.map_driver_config_to_vce_param_config(
    LT_RE850_DRIVER_CONFIG,
    [0, 1, 2, 3, 5],
    [4],
)
...
ipcMain.handle(`${DEVICE_MODEL}_get_vce_config`, () => LT_RE850_VCE_CONFIG);
...
function lt_re850_cmd_exec(cmd: string) {
...
export function init_lt_re850_serial_adapter(_main_window: BrowserWindow) {
    ...
    ipcMain.on(`${DEVICE_MODEL}_serial_port_connect`, (_, data) => {
        ...
        serial_adapter = new SerialAdapter(port_name, new LtdDriver([0x23, 0x24], LT_RE850_DRIVER_CONFIG), DEVICE_ERROR_MSG_TYPE, DEVICE_ERROR_MSG_MAP, mw_ipc_handler, mw_logger, DEVICE_MODEL);
