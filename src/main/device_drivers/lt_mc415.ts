...
const LT_MC415_DEVICE_ERRORS: LTBusDeviceErrorMsg[] = [
    {
        error_code: 0x01,
        error_text: 'Level Set Point out of Range',
        user_ack: false,
    },
];

const device_errors_map: Record<number, LTBusDeviceErrorMsg> = {};
LT_MC415_DEVICE_ERRORS.forEach(derm => device_errors_map[derm.error_code] = derm);

let main_window: BrowserWindow | null = null;
let lt_bus_driver: LtBusDriver | null = null;

ipcMain.handle(`${DEVICE_MODEL}_get_device_config`, () => LT_MC415_DRIVER_CONFIG);
ipcMain.handle(`${DEVICE_MODEL}_get_device_cmd_help`, () => LT_MC415_DEVICE_CMD_HELP);
...
const LT_MC415_DEVICE_CMD_HELP: string[] = [
    '================================================================================================================================',
...
const __pool_data_size = (LT_MC415_DRIVER_CONFIG.length - __pool_filter_set.size) * 4 + __pool_filter_set.size * 2;
const __f32_pool_msg_config = LT_MC415_DRIVER_CONFIG.filter(x => !__pool_filter_set.has(x.msg_name));
const __u16_pool_msg_config = LT_MC415_DRIVER_CONFIG.filter(x => __pool_filter_set.has(x.msg_name));
...
export function init_lt_mc415_serial_adapter(_main_window: BrowserWindow) {
