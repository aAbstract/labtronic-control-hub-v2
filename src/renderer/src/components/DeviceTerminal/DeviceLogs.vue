...
onBeforeMount(() => {
    subscribe('add_sys_log', 'add_sys_log_func', args => {
    ...
    window.electron?.ipcRenderer.on('add_sys_log', (_, data) => post_event('add_sys_log', data));
...
<template>
    <div id="device_logs_cont">
        <div v-for="log in logs_to_render" class="log_msg">
            <span v-if="log.level">[{{ log.datetime.split('T')[1] }}]</span>
