...
def round_to_nearest_tenth(number,i=1):
    return round(number * 10**i) / 10**i
...
def us_check_chx_device_state(driver: webdriver.Chrome) -> int:
    ...
    #   grap components
    #   the dropdown button arrow of the mode dropdown is the last child of the dropdown which has the id pv_id_12
    mode_drop_down = try_get_elem(driver, "#pv_id_12 > :last-child")
    ...
    calibration_option = try_get_elem(driver, "#pv_id_12_0")
    ...
    for i in range(1):
        ...
        for option_order in range(len(options_values)):
            js_click(driver, mode_drop_down)
            time.sleep(0.1)
            experiment_option = try_get_elem(driver, "#pv_id_12_1")
            ...
            js_click(driver, sample_shape_options[option_order])

            l_value = options_values[ option_order]
            t1_val = random.randint(1, 10)
            t2_val = random.randint(1, 10)
            t_amb_val = random.randint(1, 10)
            t_c_val = random.randint(1, 10)
            ...
            q_cond_val = round_to_nearest_tenth(p_heater_val - q_l_val)
            delta_t_val = round_to_nearest_tenth(t1_val - t2_val)
            Lambda_value = round_to_nearest_tenth(10/ (math.pi*delta_t_val) * l_value *  (p_heater_val -C_f*(t_h_val - t_amb_val)))
            ...
            if check_chx_device_state(driver, lt_ht103_vspi, func_id, [
                ...
                print(get_chx_device_state(driver),t1_val,t2_val,t_amb_val,t_c_val,t_h_val,p_heater_val,q_l_val,q_cond_val,Lambda_value,delta_t_val)
                return 1
...
def us_disabled_sample_shape(driver: webdriver.Chrome) -> int:
    ...
    js_click(driver, mode_drop_down)
    expremental_option = try_get_elem(driver, "#pv_id_12_1")
...
def us_calibration_routine(driver: webdriver.Chrome) -> int:
    ...
    for i in range(number_of_points):
        ...
        ql_arr.append(round_to_nearest_tenth(C_F * (th - tamp),2))
        
        qcond_arr.append(round_to_nearest_tenth( p_heater_val - C_F * (t_h_val - t_amb_val),2))
    ...
    while p_peltier_slider_value !=p_peltier_target_value:
       ...
       # Move p_heater_peltier by a small amount
        if p_peltier_slider_value<p_peltier_target_value:
          p_peltier_actions.drag_and_drop_by_offset(p_peltier_analog_slider, 10, 0).perform()
        if p_peltier_slider_value>p_peltier_target_value:
          p_peltier_actions.drag_and_drop_by_offset(p_peltier_analog_slider, 0, 10).perform()
        p_peltier_slider_value = int(driver.execute_script("return arguments[0].getAttribute('aria-valuenow')", p_peltier_analog_slider))
