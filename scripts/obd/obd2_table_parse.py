...
target_count = sum([1 for line in lines if '{' in line and '}' in line]) - 5
...
for line in lines:
    ...
    obd2_msg_pid = re.findall(r'pid: "([0-9ABCDEF]+)"', line)
    if not obd2_msg_pid:
        continue
    obd2_msg_pid: str = obd2_msg_pid[0]

    obd2_msg_name = re.findall(r'name: "([a-z0-9_]+)"', line)
    if not obd2_msg_name:
        continue
    obd2_msg_name: str = obd2_msg_name[0]

    obd2_msg_unit = re.findall(r'unit: "(.*)"', line)
    if not obd2_msg_unit:
        continue
    obd2_msg_unit: str = obd2_msg_unit[0]

    obd2_msg_size = re.findall(r'bytes: ([0-9]{1})', line)
    if not obd2_msg_size:
        continue
    obd2_msg_size = int(obd2_msg_size[0])

    obd2_msg_desc = re.findall(r'description: "(.*)"', line)
    if not obd2_msg_desc:
        continue
    obd2_msg_desc: str = obd2_msg_desc[0].split('",')[0]

    obd2_msg_obj_line = f"Obd2Msg(mode=Obd2Mode.REAL_TIME, pid=Obd2PidCode.{obd2_msg_name.upper()}, name='{obd2_msg_name}', size={obd2_msg_size}, unit='{obd2_msg_unit}', description='{obd2_msg_desc}'),"
    print(obd2_msg_obj_line)
    found_count += 1
