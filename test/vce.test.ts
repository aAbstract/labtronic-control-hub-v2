...
test('VirtualComputeEngine_patch_sequence_number_mismatch', () => {
    ...
    vce_vars_device_msgs.forEach((dmsg, idx) => {
        const vce_res = vce.load_device_msg(dmsg);
        if (idx !== 2) {
            expect(vce_res.err).toBeDefined();
            expect(vce_res.err).toBe('Loaded Var into VM Context');
            return;
        }
        expect(vce_res.err).toBeDefined();
        expect(vce_res.err).toBe('Patch Sequence Number Mismatch');
    });
});
...
test('VirtualComputeEngine_invalid_MsgType_sequence', () => {
    ...
    vce_vars_device_msgs.forEach((dmsg, idx) => {
        ...
        expect(vce_res.err).toBeDefined();
        expect(vce_res.err).toBe('Invalid MsgType Sequence');
    });
});
...
test('VirtualComputeEngine_success', () => {
    ...
    vce_vars_device_msgs.forEach((dmsg, idx) => {
        ...
        // validate vm output
        expect(vce_res.ok).toBeDefined();
        expect(vce_res.ok).toEqual({
            '$E': 2,
            '$I': 100,
            '$P': 36,
            '$T': 12,
        ...
        expect(tmp_ipc_bus[0][0]).toBe('LT_CH000_device_msg');
        expect(tmp_ipc_bus[1][0]).toBe('LT_CH000_device_msg');
